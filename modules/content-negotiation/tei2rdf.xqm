...
declare function tei2rdf:relations($rec, $id){
    ...
    return 
        ...
        else if($rec/descendant::tei:listRelation/tei:relation[@ref= 'foaf:depicts']) then ()
        ...
        if($rel/@mutual) then 
            ...
            return 
                if(contains($id,'/spear/')) then 
                    (tei2rdf:create-element('rdf:type', (), tei2rdf:expand-namespace($element-name), ()),
                     tei2rdf:create-element('snap:bondWith', (), $o, ()))
                else if($element-name = 'foaf:depicts') then
                    tei2rdf:create-element($element-name, (), $s, ())
                else tei2rdf:create-element($element-name, (), $o, ())
   )
};
...
return  
    ...
    (element { xs:QName($resource-class) } {(
                ...
                tei2rdf:create-element('rdf:type', (), tei2rdf:rec-type($rec), ()),
                (: TCADRT types :)
                if($rec//tei:listRelation/tei:relation[@ana = 'building-type']) then
                    tei2rdf:create-element('rdf:type', (), string($rec//tei:listRelation/tei:relation[@ana = 'building-type']/@passive), ())
                else if($rec//tei:listRelation/tei:relation[@ana = 'site-type']) then
                    tei2rdf:create-element('rdf:type', (), string($rec//tei:listRelation/tei:relation[@ana = 'site-type']/@passive), ())
                else (),
                tei2rdf:rec-label-and-titles($rec, 'rdfs:label'),
