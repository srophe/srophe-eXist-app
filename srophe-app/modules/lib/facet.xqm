...
declare function facet:viewable-online($results as item()*, $facet-definitions as element(facet:facet-definition)*) as element(facet:key)*{
    let $r := $results[descendant::tei:idno[@type='URI'][not(matches(.,'^(http://biblia-arabica.com|https://www.zotero.org|https://api.zotero.org|http://www.worldcat.org)'))] or descendant::tei:ref/@target[not(matches(.,'^(http://biblia-arabica.com|https://www.zotero.org|https://api.zotero.org)'))]]
    return 
        ...
        return 
            ...
            if($facet-value != '') then 
                if($facet/facet:range) then
                    ...
                    "[descendant::tei:idno[@type='URI'][not(matches(.,'^(http://biblia-arabica.com|https://www.zotero.org|https://api.zotero.org|http://www.worldcat.org)'))] or descendant::tei:ref/@target[not(matches(.,'^(http://biblia-arabica.com|https://www.zotero.org|https://api.zotero.org)'))]]"    
